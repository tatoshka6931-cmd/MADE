...
form.addEventListener('submit', (e) => {
  ...
  if (!email) return;
  // Reflect the email in the URL so the page itself becomes shareable.
...
function renderPortfolio(data) {
  if (!data.projects || data.projects.length === 0) {
    resultsEl.innerHTML = `
      <div class="empty-frame" style="margin-top: 32px;">
        No projects logged yet for ${escapeHtml(data.name)}. Head to the photobox to submit your first one.
      </div>`;
    return;
  }
  ...
  data.projects.forEach((project, i) => {
    resultsEl.appendChild(renderRoll(project, data.projects.length - i));
  });
}
...
function renderRoll(project, rollNumber) {
  ...
  if (project.photos.length === 0) {
    ...
    project.photos.forEach((photo) => {
      frames.appendChild(renderFrame(photo));
    });
...
// Auto-load if the URL already has ?email=... (this is what makes the
// page shareable — copy the URL after a lookup and send it on).
(function init() {
